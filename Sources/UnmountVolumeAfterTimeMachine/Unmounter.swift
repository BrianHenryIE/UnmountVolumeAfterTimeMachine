...
class Unmounter {
  ...
  func unmount(volume: String) {
    ...
    guard
      let urlEncodedVolume = volume.addingPercentEncoding(withAllowedCharacters: .urlQueryAllowed)
    else {
      return
    }
    ...
    diskUrls?.forEach({
      ...
      if volumePathMatcher == $0.absoluteString {
        ...
        guard
          let disk: DADisk = DADiskCreateFromVolumePath(kCFAllocatorDefault, session, $0 as CFURL)
        else {
        ...
        DADiskUnmount(
          disk, DADiskUnmountOptions(kDADiskUnmountOptionDefault),
          { _, _, _ in
